...
const SkillFilter = ({ skills, setSkills }) => {
	...
	const suggestions = useMemo(() => {
		...
		if (!q) return []
		return PRESET_SKILLS.filter(s => s.toLowerCase().includes(q) && !skills.includes(s))
	}, [query, skills])
	...
	return (
		<div className="mt-6">
			<label className="block text-sm text-gray-600 mb-2">Filter by Skills</label>
			<div className="relative">
				<input
					...
					className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-300 transition-all"
				/>
