...
const ResumeFolderDropzone = ({ onFilesSelected }) => {
	...
	const totalSize = useMemo(() => allFiles.reduce((acc, f) => acc + (f.size || 0), 0), [allFiles])

	const addFiles = useCallback((files) => {
		// Filter duplicates by webkitRelativePath + name + size
		const existingKeys = new Set(allFiles.map(f => `${f.webkitRelativePath || f.name}|${f.size}`))
		const next = []
	...
	const onFolderInput = useCallback((e) => {
	...
	return (
		<div className="max-w-4xl mx-auto px-4 py-10">
			<div className="mb-8">
				<h1 className="text-2xl md:text-3xl font-semibold text-white">Import Candidate Resumes</h1>
				<p className="text-sm text-gray-400 mt-2">Drag and drop a folder with all resumes, or choose a folder. Supported: PDF, DOC, DOCX, TXT, RTF</p>
			</div>
			...
			<div
				...
				className={`rounded-2xl border ${isDragging ? 'border-emerald-400 bg-emerald-400/5' : 'border-white/10 bg-white/5'} p-8 transition-colors`}
			>
				<div className="flex flex-col items-center gap-4 text-center">
					<div className="h-14 w-14 rounded-full bg-emerald-500/10 flex items-center justify-center">
						<span className="text-emerald-400 text-xl">📁</span>
					</div>
					<div className="text-white font-medium">Drop your resume folder here</div>
				...
				<div className="mt-8">
					<div className="flex items-center justify-between">
						<div className="text-gray-300 text-sm">
							<span className="text-white font-medium">{allFiles.length}</span> files • {bytesToReadable(totalSize)}
						</div>
						<div className="flex gap-2">
							<button onClick={clearAll} className="px-3 py-2 rounded-md text-sm bg-white/5 hover:bg-white/10 text-gray-300">Clear</button>
							<button onClick={proceed} className="px-3 py-2 rounded-md text-sm bg-emerald-600 hover:bg-emerald-700 text-white">Proceed</button>
						</div>
					</div>
					...
					<div className="mt-4 max-h-72 overflow-auto rounded-lg border border-white/10">
						<table className="w-full text-left text-sm">
							...
							<tbody>
								{allFiles.map((f, idx) => (
									<tr key={`${f.name}-${idx}`} className="border-t border-white/5">
										<td className="px-4 py-2 text-gray-200 truncate">{f.webkitRelativePath || f.name}</td>
										<td className="px-4 py-2 text-gray-400">{bytesToReadable(f.size)}</td>
									</tr>
								))}
							</tbody>
						</table>
