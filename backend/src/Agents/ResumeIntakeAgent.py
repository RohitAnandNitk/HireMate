...
class ResumeIntakeAgent:
    ...
    def process_resumes(self, resume_urls, drive_id):
        ...
        for resume_url in resume_urls:
            ...
            # Download PDF content temporarily for text extraction
            import requests, io
            ...
            human_prompt = f"Extract candidate information from the following resume text:\n\n{text}"
            messages = self.prompt_builder.build(system_prompt, human_prompt)
            response = self.llm.invoke(messages)
            ...
            try:
                ...
                drive_candidate_data = create_drive_candidate(
                ...
                print(f"Invalid JSON from LLM for {resume_url}:\n", response.content)
                continue
